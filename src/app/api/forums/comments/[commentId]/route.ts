...
import { deleteForumCommentIfAuthor } from '../../../../../lib/db';
...
export async function DELETE(_req: NextRequest, ctx: { params: Params }) {
  ...
  const res = await deleteForumCommentIfAuthor(session.userId, idNum);
  if (!res.deleted) return NextResponse.json({ ok: false, error: 'No autorizado o no existe' }, { status: 403 });
